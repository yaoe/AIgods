...
class StreamingVoiceChatbot:
    ...
    def __init__(self):
        ...
        self.vibevoice = OpenAI(base_url=os.getenv("VIBEVOICE_BASE_URL"), api_key="lala")
        self.vibevoice_model = os.getenv("VIBEVOICE_MODEL")
        self.vibevoice_path = os.getenv("VIBEVOICE_PATH")
    ...
    def _tts_worker(self):
        ...
        audio_stream = None
        sample_rate = 24000 # default, will be updated by server
        ...
        while True:
            try:
                ...
                print("Sending request to VibeVoice API for streaming... with text = ", text)
                with self.vibevoice.audio.speech.with_streaming_response.create(
                    model=self.vibevoice_model,
                    voice="ignored_when_voice_path",
                    input=text,
                    response_format="pcm",
                    extra_body={
                        "voice_path": self.vibevoice_path,
                        "stream_format": "sse",
                        "ddpm_steps": 20,
                        "cfg_scale": 1.5,
                    }
                     ...
                     # we read line by line from the raw stream and parse the SSE events
                    current_event = None
                    data_buffer = ""

                    print("About to start iterating over stream lines...")
                    print(f"Stream object type: {type(stream)}")
                    print(f"Stream has iter_lines: {hasattr(stream, 'iter_lines')}")

                    for line in stream.iter_lines():
                        print(f"[SSE] Received line: {repr(line)}")
                        if not line:  # An empty line separates events
                            if current_event and data_buffer:
                                # Process the complete event we've just received
                                try:
                                    if current_event == 'start':
                                        meta = json.loads(data_buffer)
                                        sample_rate = int(meta.get("sample_rate", 24000))
                                        print(f"Stream started. Server sample rate: {sample_rate} Hz")
                                        ...
                                        print(f"\nAn error event occurred: {error_data}")
                                        break
